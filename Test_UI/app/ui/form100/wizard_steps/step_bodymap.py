...
Левый стрип : LesionTypeWidget (вид поражения) + LesionTypeWidget (вид сан. потерь)
Полоса       : ИЗОЛЯЦИЯ (чекбокс + цветная полоса)
Центр        : BodyMapWidget (4 силуэта из PNG, растягивается)
Правый стрип : Типы тканей (компактно) + Заметки (список NOTE_PIN аннотаций)
"""
...
# Иконки для каждого типа ткани (Unicode)
_TISSUE_ICONS: dict[str, str] = {
    "мягкие ткани":  "◈",
    "кости":         "◆",
    "сосуды":        "◉",
    "полостные раны": "◇",
...
class StepBodymap(QWidget):
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        # ── Левый стрип: виды поражений + сан. потери ────────────────────
        ...
        isolation_col.addWidget(
            self.chk_isolation, 0, Qt.AlignmentFlag.AlignHCenter
        )
        ...
        isolation_col.addWidget(
            self.isolation_bar, 1, Qt.AlignmentFlag.AlignHCenter
        )
        ...
        # ── Правый стрип: типы тканей + заметки ───────────────────────────
        right_col = QVBoxLayout()
        right_col.setContentsMargins(0, 0, 0, 0)
        right_col.setSpacing(8)

        # Типы тканей — компактный блок
        ...
        # Заметки — список NOTE_PIN аннотаций
    ...
    def _refresh_notes(self) -> None:
        """Обновить список заметок в правой панели по текущим маркерам."""
    ...
    def set_values(self, payload: dict[str, str], markers: list[dict]) -> None:
        ...
        # Обновить список заметок после загрузки маркеров
        self._refresh_notes()
