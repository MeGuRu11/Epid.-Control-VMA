...
Фоновое изображение загружается из form_100_body.png.
...
# Путь к изображению схемы тела (docs/resources/form_100_body.png)
_IMG_PATH = (
    Path(__file__).parent.parent.parent.parent
    / "docs" / "resources" / "form_100_body.png"
)


# ── Domain dataclass (дублируется здесь для изоляции виджета) ─────────────
...
class _BodyCanvas(QWidget):
    ...
    # Кешируем загруженный пиксмап на уровне класса (один раз)
    ...
    def _canvas_h(self) -> float:
        """Высота зоны рисования (без полосы подписей внизу)."""
        return float(self.height()) - 22

    def _sil_rect(self, silhouette: str) -> QRectF:
        """Полный прямоугольник слота — совпадает с четвертью изображения."""
    ...
    def _body_hit_rect(self, sil: str) -> QRectF:
        """Примерная зона попадания в тело внутри слота."""
    ...
    def _hit_body(self, pos: QPointF) -> tuple[str, float, float] | None:
        """Возвращает (silhouette, nx, ny) если pos внутри активного тела."""
    ...
    def paintEvent(self, event) -> None:  # noqa: ARG002
        ...
        # Тёмный фон (совпадает с bg изображения)
        p.fillRect(self.rect(), QColor("#1A1E24"))

        # Изображение тела — растягиваем точно по ширине × высоте холста
        ...
        # Предпросмотр при наведении (перед размещением)
        ...
        # Всплывающая карточка для NOTE_PIN при наведении
        if (
    ...
    def _draw_symbol(
        ...
        *,
        ...
        """Рисует символ аннотации заданным цветом."""
        p.save()
    ...
    def _draw_note_bubble(self, p: QPainter, pos: QPointF, text: str) -> None:
        """Всплывающая карточка с текстом заметки рядом с NOTE_PIN."""
        p.save()
        font = p.font()
        font.setPointSize(9)
        p.setFont(font)
...
class BodyMapWidget(QWidget):
    ...
    API:
      set_markers / markers / pop_marker / clear_markers /
      set_markers_enabled / set_kind / set_gender / gender /
      annotations / set_payload / payload
    """
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        self._kind_group.buttons()[0].setChecked(True)  # type: ignore[union-attr]
    ...
    def template_path_text(self) -> str:
        return str(_IMG_PATH) if self.has_template() else "изображение не найдено"
