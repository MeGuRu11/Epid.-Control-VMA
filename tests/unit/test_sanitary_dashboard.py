...
def test_sanitary_dashboard_updates_selection_context_and_opens_history(monkeypatch, qtbot, qapp) -> None:
    ...
    assert "border-left: 4px" in dashboard._dep_cards[0].styleSheet()
    selected_cards = [card for card in dashboard._dep_cards if "border-left: 4px" in (card.styleSheet() or "")]
