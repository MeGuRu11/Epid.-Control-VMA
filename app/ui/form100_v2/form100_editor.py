    ...
    def _build_data_payload(self) -> dict[str, Any]:
        ...
        payload = Form100DataPayloadInput(
            stub=Form100StubPayloadInput(
                ...
                issued_time=_to_storage_time(self.stub_issued_time.time()),
                rank=self.stub_rank.text().strip(),
                unit=self.stub_unit.text().strip(),
                full_name=self.stub_full_name.text().strip(),
                id_tag=self.stub_id_tag.text().strip(),
                injury_date=_to_storage_date(self.stub_injury_date.date()),
                injury_time=_to_storage_time(self.stub_injury_time.time()),
                evacuation_method=self.stub_evac_method.currentData(),
                evacuation_dest=self.stub_evac_dest.currentData(),
                med_help_underlined=stub_med_help_underline,
                antibiotic_dose=self.stub_antibiotic_dose.text().strip(),
                pss_pgs_dose=self.stub_pss_pgs_dose.text().strip(),
                toxoid_type=self.stub_toxoid_type.text().strip(),
                antidote_type=self.stub_antidote_type.text().strip(),
                analgesic_dose=self.stub_analgesic_dose.text().strip(),
                transfusion=self.stub_transfusion.isChecked(),
                immobilization=self.stub_immobilization.isChecked(),
                tourniquet=self.stub_tourniquet.isChecked(),
                diagnosis=self.stub_diagnosis.toPlainText().strip(),
            ),
            main=Form100MainPayloadInput(
                full_name=self.main_full_name.text().strip(),
                unit=self.main_unit.text().strip(),
                id_tag=self.main_id_tag.text().strip(),
                rank=self.main_rank.text().strip(),
                issued_place=self.main_issued_place.text().strip(),
                issued_date=_to_storage_date(self.main_issued_date.date()),
            ...
            medical_help=Form100MedicalHelpPayloadInput(
                antibiotic=self.mp_antibiotic.isChecked(),
                ...
                analgesic_dose=self.mp_analgesic_dose.text().strip(),
                transfusion_blood=self.mp_transfusion_blood.isChecked(),
            ...
            flags=Form100FlagsPayloadInput(
                emergency=self.flag_emergency.isChecked(),
                radiation=self.flag_radiation.isChecked(),
                sanitation=self.flag_sanitation.isChecked(),
            ),
        )
        return build_form100_data_payload(payload)
