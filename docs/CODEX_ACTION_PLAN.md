...
#### Автоматизированные проверки (Codex)

| Проверка | Команда |
|----------|---------|
| Линтер | `ruff check app tests` |
| Типы | `python -m mypy app tests` |
| Архитектура слоёв | `python scripts/check_architecture.py` |
| Все тесты | `python -m pytest -q --tb=short` |
| Компиляция | `python -m compileall -q app tests scripts` |
| Alembic | `python -m alembic check` |
| Мохибаке | `python scripts/check_mojibake.py` |
| Артефакты | `python scripts/validate_generated_artifacts.py tmp_run/generated_app_outputs_<latest>` |

#### Функциональный аудит (ручной)

| Область | Что проверить |
|---------|--------------|
| Form100 | Создание, редактирование, подписание, экспорт PDF/ZIP — дата рождения, bodymap метки, diff-баннер |
| Аналитика v1 | При `use_analytics_v2=False` — всё работает как до редизайна |
...
Пройтись по всем чек-боксам в `docs/specs/SPEC_analytics_redesign.md` (130+ пунктов) — отметить пройденные, зафиксировать непройденные.
...
Зафиксировать процент покрытия. Найти критические модули с покрытием < 40% и добавить тесты.

#### Проверка производительности

На синтетической БД с 1000+ пробами:
- Открытие раздела Аналитика v2 → KPI отображаются < 2 сек.
- Смена фильтра → обновление < 1 сек.
- Heatmap 10×10 → рендер < 500 мс.

---

### Результат аудита

Codex формирует отчёт `docs/audit_report_v1_1_0.md`:
...
## Покрытие тестами
- Общее: X%
- Критические модули < 40%: список

## Найденные проблемы
- Список с приоритетом (Critical / High / Medium / Low)
...
```
chore: comprehensive system audit and regression check for v1.1.0
```
