...
> **Статус**: реализовано в текущем контуре Form100 V2.
...
> **Примечание**: этот файл хранит исходные требования v2.2. Актуальное поведение системы фиксируется в `docs/context.md`, `docs/tech_guide.md` и `docs/progress_report.md`.

## Актуализация На 2026-03-04
- Реализация Form100 V2 завершена и используется в production-контуре.
- PDF-отчет Form100 формируется как структурированный `A4` документ (`app/infrastructure/reporting/form100_pdf_report_v2.py`).
- Bodymap в текущей реализации работает на двух рабочих силуэтах (`male_front`, `male_back`); женские обозначения нормализуются в эти же контуры.
- Для bodymap в PDF используется шаблон изображения `form_100_bd.png`; при его отсутствии включается векторный fallback.
...
│   └── Женщина (F)
│
├── QGraphicsScene  (рабочая область для 2 силуэтов: front/back)
│   │
│   ├── Слой силуэтов (2 панели: front/back; женские ключи нормализуются к этим силуэтам)
│   │   ├── male_front (левая панель)
│   │   └── male_back  (правая панель)
│   │
...
class AnnotationData:
    ...
    y: float               # 0.0–1.0
    silhouette: str        # 'male_front' | 'male_back' (женские значения нормализуются)
    note: str = ""         # текстовая заметка (для NOTE_PIN)
```

#### Поведение:
- **Отображение**: отображаются два силуэта (`male_front`, `male_back`), расположенные горизонтально.
- **Активные силуэты**: оба силуэта доступны для разметки; отдельные женские панели не используются.
- **Визуальная обратная связь**: подсветка активного инструмента и hover-preview на текущем силуэте.
...
│   ├── BodyMapWidget          схема тела — 2 силуэта (центр, растягивается)
│   │   └── QComboBox (пол) сохраняется для совместимости данных
│   └── QGroupBox              «Медицинская помощь» (правый столбец, ~220px)
│
└── QGroupBox «Идентификация»  (под верхним рядом, все поля ФИО/жетон/дата)
```

**Примечание**: BodyMapWidget занимает центральную область, используются 2 панели (вид спереди/сзади). Масштабирование выполняется пропорционально.

---

## VI.6 PDF-генерация (`form100_pdf_report_v2.py`)

**Цель**: формировать структурированный читаемый PDF-отчет формы 100 со всеми заполненными полями и bodymap.

### Принципы:

1. Использовать `reportlab.platypus` (Paragraph, Table, Image).
2. Размер страницы: **A4** (`reportlab.lib.pagesizes.A4`).
3. Отчет строится секциями (идентификация, поражения, bodymap, медпомощь, эвакуация) через `platypus`-таблицы.
4. Схема тела в PDF:
   - основной путь: шаблон `app/image/main/form_100_bd.png` + наложение аннотаций;
   - fallback: векторная схема, если шаблон недоступен.
5. Путь выходного PDF задается вызывающим сервисом (`file_path`).
6. При экспорте через `Form100ServiceV2` и `ReportingService` сохраняются SHA256 и запись артефакта в историю отчетов.
