...
- Проведен ручной технический аудит проекта (кодировка/язык/логика/модули/БД):
  ...
  - по паттернам «крякозябр» (`U+FFFD`, `Ð`, `Ñ`, `Ã`, `Â`) совпадений не найдено;
...
- Дополнительно подтверждена чистота по рабочему контуру:
  - в `app/` и `tests/` больше нет найденных паттернов mojibake (`РР`, `РЎР`, `Ð`, `Ñ`, `Ã`, `Â`, `U+FFFD`) в `*.py`.
...
- Проведен полный технический аудит кодовой базы на предмет наличия проблем с кодировкой ("кракозябр") и экранированных кириллических символов:
  - Выполнено сканирование `*.py`, `*.md`, `*.json`, `*.yaml` на наличие битых символов (таких как `Ð`, `Ñ`, `Ã`, `Â`, ``).
...
- Исправлена критическая ошибка в NSIS-установщике (`scripts/installer.nsi`):
  - Секция `Section "Удаление"` ошибочно была обычной секцией установки, а не секцией деинсталляции.
  - NSIS выполнял её при установке, немедленно удаляя только что созданные ярлыки (Рабочий стол / Пуск).
  - Исправлено: переименована в `Section "un.Uninstall"`, теперь удаление выполняется только из `Uninstall.exe`.

### 2026-03-11: Комплексный аудит кода (Горизонтальный и Вертикальный)

- Проведен глубокий аудит слоев приложения (Clean Architecture).
- Проверено разделение прав доступа (RBAC) между `admin` и `operator` на уровнях UI и сервисов. Уязвимостей не выявлено.
- Проверен механизм бизнес-валидаций (Bodymap) и система контроля сессий (Unit of Work).
- Сформирован итоговый отчет в файле `docs/code_audit_findings.md`.
