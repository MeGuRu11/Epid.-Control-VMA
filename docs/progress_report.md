...
## S4.2 Этап 2 — KPI-cards и Overview tab

**Коммит:** `feat: S4.2 Этап 2 — KPI cards with trend indicators on overview tab`
**Статус:** закрыто
**Дата:** 2026-05-15

- Добавлены `TrendIndicator` и `KpiCard` для обзорной вкладки Analytics v2 без внешних иконочных зависимостей.
- В `OverviewTab` добавлен первый ряд из четырёх KPI-карточек: госпитализации, случаи ИСМП, положительные лабораторные результаты, превалентность.
- KPI обновляются из существующих `get_aggregates()`, `get_ismp_metrics()` и `compare_periods()`; для ИСМП и превалентности тренд оставлен `—` до отдельного `compare_ismp` в следующем этапе.
- Стили `#kpiCard`, `#kpiIcon`, `#kpiTitle`, `#kpiValue`, `#kpiTrend*` добавлены в `app/ui/theme.py`; inline `setStyleSheet()` в новых UI-виджетах не используется.
- Добавлены unit-тесты для KPI-карточки, индикатора тренда и наличия четырёх KPI-карточек на `OverviewTab`.
...
- GREEN: `python -m pytest tests/unit/test_kpi_card.py tests/unit/test_trend_indicator.py tests/unit/test_analytics_v2_structure.py -q` — `20 passed`, `2 warnings`.
- `ruff check app tests` — pass.
- `python -m mypy app tests` — pass (`366 source files`).
- `python scripts/check_architecture.py` — pass.
- `python -m pytest -q --tb=short` — pass (`729 passed`, `3 warnings`).
- `python -m pytest tests/unit/test_kpi_card.py tests/unit/test_trend_indicator.py -v` — `10 passed`, `2 warnings`.
- `python -m compileall -q app tests` — pass.

---

## S4.2 Этап 3 — Sparklines и drill-down

**Коммит:** `feat: S4.2 Этап 3 — sparklines and drill-down navigation on KPI cards`
**Статус:** закрыто
**Дата:** 2026-05-15

- Добавлен `Sparkline` — компактный 70×28 мини-график на `QPainter` без внешних библиотек.
- `KpiCard` получил опциональный sparkline: для карточек госпитализаций и положительных результатов включён, для ИСМП и превалентности выключен.
- `OverviewTab.refresh()` теперь переиспользует один результат `get_trend()` и передаёт trend rows одновременно в общий график и KPI sparklines.
- Добавлены константы индексов вкладок в `app/ui/analytics/tabs/__init__.py`.
- Добавлен drill-down: клики по KPI ИСМП/превалентность переключают на вкладку ИСМП, клик по положительным — на вкладку Микробиология.
- `AnalyticsViewV2` подключает `OverviewTab.drill_down_requested` напрямую к `QTabWidget.setCurrentIndex`.
- `analytics_view.py` v1 не изменялся; оси/легенды/tooltips для sparkline не добавлялись.
...
- GREEN: `python -m pytest tests/unit/test_sparkline.py tests/unit/test_kpi_card.py tests/unit/test_analytics_v2_structure.py -q` — `23 passed`, `2 warnings`.
- `ruff check app tests` — pass.
- `python -m mypy app tests` — pass (`368 source files`).
- `python scripts/check_architecture.py` — pass.
- `python -m pytest -q --tb=short` — pass (`739 passed`, `3 warnings`).
- `python -m pytest tests/unit/test_sparkline.py -v` — `5 passed`, `2 warnings`.
- `python -m pytest tests/unit/test_kpi_card.py -v` — `6 passed`, `2 warnings`.
- `python -m compileall -q app tests` — pass.

---

## S4.2 Этап 4 — Microbiology tab: heatmap, resistance, quick chips

**Коммит:** `feat: S4.2 Этап 4 — microbiology tab with heatmap, resistance pattern and quick filter chips`
**Статус:** закрыто
**Дата:** 2026-05-15

- В слой аналитики добавлено поле `ris` в `AnalyticsSampleRow`; `AnalyticsRepository.search_samples()` теперь выбирает первый RIS из `lab_abx_susceptibility`, а `AnalyticsService.search_samples()` прокидывает его в DTO.
- В `AnalyticsController` добавлены вычисления для вкладки микробиологии:
  - `get_heatmap_data()` строит матрицу `department_name × microorganism` по положительным пробам;
  - `get_resistance_data()` строит матрицу `microorganism × antibiotic` с подсчётом `S/I/R/total`.
- Добавлены виджеты Analytics v2:
  - `Heatmap` / `HeatmapCell` для тепловой карты отделений и микроорганизмов;
  - `ResistanceGrid` для процента резистентности;
  - `QuickFilterChips` для быстрых фильтров микробиологии.
- `MicrobiologyTab` теперь показывает быстрые chips, существующий топ микроорганизмов, heatmap и resistance grid; drill-down по ячейке оставлен как no-op до Этапа 6.
- Новые стили добавлены через `app/ui/theme.py`; inline `setStyleSheet()` в новых UI-файлах не используется.
- `analytics_view.py` v1 не изменялся; Alembic-миграция не создавалась.
...
- Baseline: `python -m pytest -q --tb=no` — pass (`739 passed`, `3 warnings`; повторён с увеличенным таймаутом после первого timeout).
- RED: `python -m pytest tests/unit/test_heatmap.py tests/unit/test_resistance_grid.py tests/unit/test_quick_filter_chips.py tests/integration/test_analytics_ris_in_search.py -q` — expected `13 failed` по отсутствующим виджетам, `ris` и методам controller.
...
## S4.2 Этап 5 — ИСМП tab: DonutChart, KPI-cards, Bar отделений

**Коммит:** `feat: S4.2 Этап 5 — ISMP tab redesign with donut chart and department bar`
**Статус:** закрыто
**Дата:** 2026-05-15

- В `AnalyticsRepository` добавлен `get_ismp_by_department()` — группировка ИСМП-случаев по отделениям с сортировкой по убыванию.
- В `AnalyticsService` и `AnalyticsController` добавлен делегирующий метод `get_ismp_by_department()`.
- Добавлен `app/ui/analytics/widgets/donut_chart.py`:
  - `DonutCanvas` рисует кольцевую диаграмму через `QPainter`;
  - `DonutChart` добавляет легенду справа;
  - `IsmpDepartmentBar` показывает горизонтальные бары топ-N отделений.
- `IsmpTab` переработан: 5 KPI-карточек, DonutChart по типам ИСМП, bar отделений и существующая таблица типов ИСМП для детализации.
- Стили для donut-легенды и `QProgressBar#ismpDeptBar` добавлены в `app/ui/theme.py`; inline `setStyleSheet()` не добавлялся.
- `analytics_view.py` v1 и другие вкладки не изменялись.
...
- RED: `python -m pytest tests/unit/test_donut_chart.py tests/integration/test_analytics_ismp_by_dept.py tests/unit/test_analytics_v2_structure.py -q` — expected `11 failed` по отсутствующим `donut_chart.py`, `get_ismp_by_department()` и новой структуре `IsmpTab`.
...
## S4.2 Этап 6 — Search + Reports tabs polish, color-coded badges

**Коммит:** `feat: S4.2 Этап 6 — color-coded badges in search results and reports tab actions`
**Статус:** закрыто
**Дата:** 2026-05-15

- Во вкладке Analytics v2 `SearchTab` добавлена колонка `Рост` с локализованным значением через `format_growth_flag()`.
- Для результатов поиска отключены alternating rows; положительные пробы (`growth_flag == 1`) подсвечиваются мягким красным фоном `#FEE2E2`, отрицательные строки остаются без ручной заливки.
- Во вкладке `ReportsTab` добавлена кнопка `Проверить хеши`, запускающая загрузку истории с `verify_hash=True`.
- История отчётов теперь цветит строки по статусу верификации: `OK` — зелёный `#EAF3DE`, mismatch/missing/error — красный `#FECACA`.
- Tooltip пути артефакта в колонке 7 сохранён.
- Стиль `QTableWidget` с `gridline-color: {COL["border_soft"]}` уже был в `theme.py`, дублирование не добавлялось.
...
## S4.2 Этап 7 — стиль и polish: section frames и empty states

**Коммит:** `feat: S4.2 Этап 7 — analytics v2 styling polish with section frames and empty states`
**Статус:** закрыто
**Дата:** 2026-05-15

- Добавлен helper `make_section_frame()` в `app/ui/analytics/view_utils.py`: создаёт `QFrame#sectionFrame` с `QLabel#sectionTitle`.
- Все `QGroupBox` во вкладках Analytics v2 и `FilterBar` заменены на `sectionFrame`; `app/ui/analytics/analytics_view.py` v1 не изменялся.
- Добавлен виджет `EmptyState` для пустых состояний Analytics v2 без inline `setStyleSheet()`.
- Empty states подключены к `OverviewTab`, `MicrobiologyTab`, `IsmpTab`, `SearchTab`, `ReportsTab`.
- KPI-ряды в `OverviewTab` и `IsmpTab` переведены с `QHBoxLayout` на `QGridLayout` с растяжением колонок.
- В `theme.py` добавлены стили для `QFrame#sectionFrame`, `QFrame#emptyState`, `QLabel#emptyStateText`, `QLabel#emptyStateHint`.
...
## S4.2 Этап 8 — удаление Analytics v1 и завершение миграции на v2

**Коммит:** `chore: S4.2 Этап 8 — remove analytics v1, make v2 default`
**Статус:** закрыто
**Дата:** 2026-05-15

- `MainWindow` теперь безусловно создаёт `AnalyticsViewV2`; ветка `AnalyticsSearchView` удалена.
- Удалён `app/ui/analytics/analytics_view.py` с v1-реализацией.
...
- Раздел 2: KPI, тренды, ИСМП, heatmap и resistance покрыты v2-кодом и тестами `test_analytics_v2_structure.py`, `test_analytics_chart_data.py`, `test_kpi_card.py`, `test_trend_indicator.py`, `test_sparkline.py`, `test_heatmap.py`, `test_resistance_grid.py`, `test_donut_chart.py`, `test_analytics_ris_in_search.py`, `test_analytics_ismp_by_dept.py`.
- Раздел 3: поиск и цветовые бейджи покрыты `SearchTab` и `test_search_tab_badges.py`.
- Раздел 4: экспорт XLSX/PDF остаётся через `SearchTab -> AnalyticsController -> ReportingService`; сервисный экспорт покрыт существующими analytics report tests.
- Раздел 5: история отчётов и проверка хешей покрыты `ReportsTab`, `report_history_helpers` и `test_reports_tab_verify.py`.
- Ручной GUI-прогон не выполнялся; проверка выполнена по коду и автоматическим тестам.
...
- `app/ui/main_window.py`
- `app/ui/settings/settings_dialog.py`
- `app/ui/analytics/analytics_view.py` (удалён)
- `tests/unit/test_analytics_chart_data.py`
- `tests/unit/test_dropdown_indicators.py`
- `tests/unit/test_ui_smoke.py`
- `tests/unit/test_user_preferences_analytics_flag.py` (удалён)
- `docs/progress_report.md`
- `docs/session_handoff.md`
...
- Checkpoint after default `use_analytics_v2=True`: `python -m pytest -q --tb=short` — `778 passed`, `3 warnings`.
- Checkpoint after unconditional `AnalyticsViewV2` in `MainWindow`: `python -m pytest -q --tb=short` — `778 passed`, `3 warnings`.
- Checkpoint after deleting `analytics_view.py`: `python -m pytest -q --tb=short` — `778 passed`, `3 warnings`.
- Checkpoint after deleting feature flag: `python -m pytest -q --tb=short` — `774 passed`, `3 warnings`.
- `ruff check app tests` — pass.
- `python -m mypy app tests` — pass (`380 source files`).
- `python scripts/check_architecture.py` — pass.
...
## 2026-05-16 — fix: HomeView layout correctly recalculates on first maximized show

**Коммит:** `fix: HomeView layout correctly recalculates on first maximized show`
**Статус:** готово к коммиту

- Диагностика показала, что `HomeView.showEvent` и `resizeEvent` вызываются при первом `showMaximized`, но окно получает завышенный первичный минимум.
- Корневая причина была не в `HomeView`, а в `TransitionStack`: стандартный `QStackedWidget` считал `minimumSizeHint/sizeHint` по всем страницам, включая скрытую `AnalyticsViewV2` с `minimumSizeHint = 957×1820`.
- Из-за этого первый maximized-показ отдавал `HomeView` размер около `1138×1820`; hero/utility растягивались до `739px`, а внутренние `QLabel` получали высоты `129–130px`.
- После ручного minimize/maximize Windows уже отдавал нормальную геометрию `1707×815`, поэтому layout визуально исправлялся.
- `TransitionStack` теперь возвращает `sizeHint()` и `minimumSizeHint()` текущей страницы, с учётом её `minimumSize`, и вызывает `updateGeometry()` при animated/direct switch через `setCurrentWidgetAnimated`.
- Добавлен regression-тест: текущий `HomeView` в `TransitionStack` не должен получать минимальную высоту скрытой высокой страницы.
- Убраны все временные diagnostic `print(...)` из `HomeView`.
- Существующие тесты окна/настроек синхронизированы с текущим runtime-контрактом:
  - dummy-окно в тестах initial size поддерживает `isMaximized()` / `isFullScreen()`;
  - fallback invalid `window_initial_state` соответствует актуальному дефолту `maximized`;
  - offscreen-проверка выравнивания logout допускает 4px разницы из-за Qt font metrics.

### Диагностика

- До фикса, полный `MainWindow`, пустой data dir/preferences:
  - до показа: `stack-min = 957×1820`, `home-min = 428×968`, `analytics-min = 957×1820`;
  - первый maximized: `window = 1138×1902`, `HomeView = 1138×1820`, hero/utility `739px`;
  - после manual minimize/maximize: `window = 1707×897`, `HomeView = 1707×815`, hero/utility `250px`.
- После фикса:
  - до показа: `stack-min = 428×968`, `analytics-min = 957×1820`;
  - первый maximized: `window = 1707×897`, `HomeView = 1707×815`, hero `1261×250`, stats `1675×296`;
  - после manual resize/maximize: та же геометрия.
...
- RED: `python -m pytest tests\unit\test_transition_stack.py::test_transition_stack_uses_current_page_minimum_size_for_initial_layout -q --tb=short` — fail (`stack.minimumSizeHint().height() == 1820` вместо текущей страницы); затем тест уточнён на реальный `HomeView`.
- GREEN: `python -m pytest tests\unit\test_transition_stack.py -q --tb=short` — pass (`3 passed`, warnings только существующие cache/pytest_asyncio).
- GUI-диагностика полного `MainWindow` после фикса: первый maximized и manual resize/maximize дали одинаковую геометрию.
- `ruff check app tests` — pass.
- `python -m mypy app tests` — pass (`380 source files`).
- `python scripts/check_architecture.py` — pass.
...
## 2026-05-17 — fix: Analytics v2 empty states, ExitDialog centering, HomeView regression on tab switch

**Коммит:** `fix: Analytics v2 empty states, ExitDialog centering, HomeView regression on tab switch`
**Статус:** готово к коммиту

- Диагностика подтвердила регрессию `HomeView` после перехода `Главная → Аналитика → Главная`: до фикса `AnalyticsViewV2` раздувал окно до `1707×1902`, а возвращённый `HomeView` получал `1707×1820`; hero/utility снова растягивались до `739px`.
- Корневая причина Home-регрессии: `TransitionStack` обновлял геометрию не на всех direct switch-путях, а `AnalyticsViewV2` всё ещё имел высокий `minimumSizeHint` из-за вложенных табов и скрытых страниц.
- Корневая причина Analytics empty state: вкладки `Overview`, `Microbiology`, `ISMP` скрывали весь контент за одним полноэкранным `EmptyState`, поэтому KPI, фильтры и структура аналитики исчезали при пустой БД.
- Корневая причина огромной высоты Analytics: обычный `QStackedWidget` внутри секций учитывал size hints скрытых графиков/таблиц даже когда показывался компактный placeholder.
- Корневая причина Exit/Logout dialog: подтверждающие диалоги полагались на платформенное позиционирование Qt и на Windows могли появляться у нижнего края экрана.
- Добавлен компактный `make_inline_placeholder(...)` и `CurrentWidgetStack`, который возвращает size hint только текущей inline-страницы.
- `OverviewTab`, `MicrobiologyTab`, `IsmpTab` переведены с полноэкранного `EmptyState` на inline placeholders внутри графиков, heatmap, resistance grid и таблиц; KPI и фильтры остаются видимыми при нулевых данных.
- `AnalyticsViewV2` оборачивает тяжёлые вкладки в `QScrollArea` и больше не навязывает главному окну высокую минимальную высоту.
- `TransitionStack` инвалидирует layout и вызывает `updateGeometry()` для новой текущей страницы при `setCurrentIndex(...)`, `setCurrentWidget(...)` и animated switch.
- `confirm_exit(...)` и `confirm_logout(...)` центрируют диалог относительно parent перед `exec()`.
- Временные diagnostic `print(...)` из `HomeView` удалены.

### Диагностика

- До фикса:
  - первый maximized: `window = 1707×897`, `HomeView = 1707×815`, hero/utility `250px`;
  - после перехода на Analytics: `window = 1707×1902`, `AnalyticsViewV2 = 1707×1820`;
  - после возврата Home: `window = 1707×1902`, `HomeView = 1707×1820`, hero/utility `739px`.
- После фикса:
  - первый maximized: `window = 1707×897`, `HomeView = 1707×815`, hero/utility `250px`;
  - Analytics: `window = 1707×897`, `AnalyticsViewV2 = 1707×815`, `minimumSizeHint = 557×665`;
  - после возврата Home: `window = 1707×897`, `HomeView = 1707×815`, hero/utility `250px`.
...
- RED: `python -m pytest tests/unit/test_analytics_v2_empty_states.py tests/unit/test_exit_dialog_position.py tests/unit/test_transition_stack.py -q --tb=short` — падали проверки отсутствующих inline placeholders, нецентрированного dialog и отсутствующего `updateGeometry()` при switch.
- GREEN targeted: `python -m pytest tests/unit/test_analytics_v2_empty_states.py tests/unit/test_analytics_chart_data.py tests/unit/test_analytics_v2_structure.py tests/unit/test_exit_dialog_position.py tests/unit/test_transition_stack.py -q --tb=short` — `60 passed`.
- `ruff check app tests` — pass.
- `python -m mypy app tests` — pass (`382 source files`).
- `python scripts/check_architecture.py` — pass.
