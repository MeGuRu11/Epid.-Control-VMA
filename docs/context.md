...
## 11) Формат обмена (исторический черновик, не использовать как текущий контракт)

> Раздел сохранён как историческая справка. Актуальный контракт обмена для Form100 V2 описан в разделе 18.
...
- Дополнение (2026-04-06): глобальный аудит и этапы очистки:
  - проведена полная инвентаризация проекта и обновлен аудит в `docs/full_audit_report.md`;
  - удалены устаревшие артефакты/конфигурации, не относящиеся к стеку Codex + mypy + ruff + pytest;
  - подтверждено устранение критичных нарушений `UI -> Infrastructure`;
  - по `alembic check` устранён schema drift (учтены FTS-исключения и выровнены индексы).
