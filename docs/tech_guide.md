...
Текущее архитектурное уточнение после аудита 2026-04-24: `Application -> Infrastructure` допускается для read-model/query/reporting-сценариев, где сервис приложения агрегирует данные через SQLAlchemy-запросы. Для новых write/use-case потоков предпочтительный путь остаётся через репозитории и сервисные границы, чтобы не расширять связность application-слоя со схемой БД.
...
Текущая ORM-политика: существующие SQLAlchemy-модели используют классический `Column(...)`. Переход на `Mapped[]` / `mapped_column()` рассматривается как отдельная миграционная задача и не должен смешиваться с точечными исправлениями без явного плана.
...
- компактное отображение закреплённых пациента и госпитализации без навигационных quick-actions.
