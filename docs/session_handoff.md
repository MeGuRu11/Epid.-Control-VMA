...
- Проанализированы текущие UI-паттерны проекта по `app/ui/theme.py`, `app/ui/runtime_ui.py`, ключевым экранам (`Главная`, `Лаборатория`, `Санитария`, `Логин`, `Первый запуск`) и UI-утилитам.
- Подтверждено, что проект уже использует единый theme-layer через `app/ui/theme.py`, objectName/property-driven QSS и запрет на inline `setStyleSheet()` вне `theme.py`.
- Выделены фактические токены и паттерны:
  - тёплая светлая палитра с mint-акцентом;
  - базовая типографика `11/12 px` с заголовками `22 px` и плотными таблицами `10 px`;
  - повторяющаяся шкала отступов `4 / 6 / 8 / 10 / 12 / 14 / 16 / 18 / 24`;
  - радиусы `4 / 6 / 8 / 10 / 12 / 16` и pill-бейджи;
  - runtime-флаги `EPIDCONTROL_UI_PREMIUM`, `EPIDCONTROL_UI_ANIMATION`, `EPIDCONTROL_UI_DENSITY`.
- Создан новый корневой файл `DESIGN.md` как лёгкая дизайн-система для будущих UI-изменений Codex.
- В `AGENTS.md` добавлены ссылки на `DESIGN.md` и прямое правило следовать ему для всех изменений интерфейса.

## Что не закончено / в процессе

- `DESIGN.md` интегрирован только на уровне документации и агентских инструкций; UI-код в этой сессии сознательно не менялся.
- Возможные точечные выравнивания по теме пока только зафиксированы как наблюдения, без внедрения.

## Открытые проблемы / блокеры

- Блокеров по документированию дизайн-системы нет.
- Визуальные мелкие расхождения остаются в коде:
  - `app/ui/analytics/charts.py` использует жёстко заданный синий `#4C78A8`, который выбивается из текущей тёплой палитры;
  - часть showcase-экранов (`login`, `first_run`, некоторые custom-painted элементы) дублирует цвета напрямую, а не через общие токены;
  - в `app/ui/form100_v2/wizard_widgets/wizard_steps/step_evacuation.py` есть `setObjectName("secondary")`, который выглядит непоследовательно рядом с общим паттерном `secondaryButton`.

## Следующие шаги

1. При следующей UI-задаче использовать `DESIGN.md` как основной контракт перед изменением `theme.py` и objectName-паттернов.
2. При необходимости точечно выровнять hardcoded UI-цвета и objectName, не запуская крупный рефакторинг темы.
3. Если `DESIGN.md` окажется полезным в нескольких сессиях подряд, при желании добавить на него ссылку в `docs/tech_guide.md`.
