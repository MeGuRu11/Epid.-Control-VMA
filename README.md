...
Версия документации: `1.1.0` (`2026-05-18`).
...
- строит Analytics v2: 5 вкладок (`Обзор`, `Микробиология`, `ИСМП`, `Поиск`, `Отчёты`), KPI, графики, heatmap, resistance grid и историю отчётов;
...
### 4. Демо-данные для проверки аналитики

Для локальной проверки непустых графиков и таблиц можно наполнить БД демонстрационными данными:

```powershell
python scripts/seed_demo_data.py          # добавить demo-данные
python scripts/seed_demo_data.py --clear  # удалить demo-данные
```

Скрипт создаёт пациентов, ЭМЗ, лабораторные пробы с RIS, ИСМП-случаи и санитарные пробы. Demo-записи помечаются префиксом `DEMO-` и очищаются отдельно от пользовательских данных.

### 5. Демо-выгрузки

Для генерации всех возможных форматов экспорта с тестовыми данными:

```powershell
python scripts/generate_sample_exports.py
```

Файлы сохраняются в `docs/sample_exports/` и не попадают в git. Опции: `--skip-seed` не перезасевает БД, `--out-dir PATH` задаёт другой каталог.
...
- `Аналитика` — Analytics v2 с общей панелью фильтров и 5 вкладками: `Обзор`, `Микробиология`, `ИСМП`, `Поиск`, `Отчёты`.
...
scripts/
  ...
  seed_demo_data.py         Demo-данные для проверки Analytics v2
```
